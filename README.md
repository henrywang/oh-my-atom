...
| New Window | `Cmd+Shift+n` | `Ctrl+Shift+n`  | `Ctrl+Shift+n` | Opens a new editor window |
...
| Save As | `Cmd+Shift+s` | `Ctrl+Shift+s`  | `Ctrl+Shift+s` | Saves the currently active file under a different name  |
| Save All | `Alt+Cmd+s` |  |  | Saves all changed files |
| Close Tab | `Cmd+w` |  `Ctrl+w` | `Ctrl+w` | Closes the currently active tab|
| Close Window | `Cmd+Shift+w` | `Ctrl+Shift+w`  | `Ctrl+Shift+w` | Closes the currently active editor window  |
...
| Command | macOS | Windows | Linux | Description |
| ------- | -------- | ------- | ----- | ----------- |
| Duplicate Lines | `Cmd+Shift+d` | `Ctrl+Shift+d` | `Ctrl+Shift+d` | Duplicates the line of the current cursor position and creates a new line under it with the same contents |
| Delete Line | `Ctrl+Shift+k` | `Ctrl+Shift+k` | `Ctrl+Shift+k` | Deletes the current line |
| Move Line Up | `Cmd+Ctrl+up` | `Ctrl+up`  | `Ctrl+up` | Moves the contents of the current cursor position up one line. If there is a line above with content, the current lines content will swap with the one above it. |
| Move Line Down | `Cmd+Ctrl+down` | `Ctrl+down`  | `Ctrl+down` | Moves the contents of the current cursor position down one line. If there is a line below with content, the line's content will swap with the one below it. |
...
| Find Previous | `Cmd+Shift+g` | `Shift+F3`  | `Shift+F3` | Toggles backward through the results of the current buffer in the file while the Find/Replace panel is active|
| Find in Project | `Cmd+Shift+f` | `Ctrl+Shift+f`  | `Ctrl+Shift+f` | Opens the Find in Project Panel |
